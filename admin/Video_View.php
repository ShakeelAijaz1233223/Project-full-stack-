...
/* ===============================
    3. DELETE VIDEO (POST Recommended)
================================ */
...
<head>
    ...
    <style>
        ...
        .delete-btn {
            position: absolute; top: 15px; right: -100px; /* Hidden on right */
            z-index: 30;
...
<body>
    <aside class="sidebar">
        <div class="mb-5 px-3">
            <h4 class="fw-bold text-white"><i class="fa-solid fa-play text-primary me-2"></i><span>Gallery</span></h4>
        </div>
        <a href="dashboard.php"><i class="fa-solid fa-house"></i> <span>Dashboard</span></a>
        <a href="add_video.php"><i class="fa-solid fa-cloud-arrow-up"></i> <span>Upload</span></a>
        <a href="logout.php" class="text-danger mt-4"><i class="fa-solid fa-right-from-bracket"></i> <span>Logout</span></a>
    </aside>

    <main class="main-content">
        <input type="text" id="search" class="search-box" placeholder="🔍 Search video titles...">
        ...
        <div class="grid" id="videoGrid">
            ...
            ?>
                    <div class="card" data-title="<?php echo strtolower($title); ?>">
                        ...
                        <div class="thumbnail">
                            <?php if (!empty($file) && file_exists($fullPath)): ?>
                                <video src="<?= $fullPath ?>" preload="metadata" playsinline></video>
                            <?php else: ?>
                        ...
                        <div class="info">
                            <span class="title text-truncate" title="<?= $title ?>"><?php echo $title; ?></span>
                            <span class="meta"><i class="fa-regular fa-folder-open me-1"></i> Library Video</span>
                        </div>  
                    </div>
            <?php 
                endwhile;
    ...
    <script>
        ...
        if (urlParams.has('deleted')) {
            Swal.fire({
                title: 'Deleted!',
                text: 'Video file has been removed.',
                icon: 'success',
                background: '#1e293b',
                color: '#f8fafc',
        ...
        document.querySelectorAll('.btn-confirm-delete').forEach(button => {
            button.addEventListener('click', function() {
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: "The video file will be permanently deleted!",
                    icon: 'warning',
