...
.custom-controls button { background:none; border:none; color:#fff; cursor:pointer; }
.progress { flex:1; height:5px; accent-color:var(--accent); cursor:pointer; }

/* Titles & Info */
.title { font-size:1rem; font-weight:700; margin:0; color:#fff; }
.meta-info { display:flex; gap:8px; margin:5px 0 10px; font-size:0.75rem; flex-wrap:wrap; }
.meta-info span { background: rgba(255,255,255,0.08); padding:2px 8px; border-radius:5px; color:var(--text-muted); }
.stars-display { color:#ffd700; font-size:0.8rem; margin-bottom:12px; }
.rev-btn {
    width:100%; padding:8px; border-radius:10px; border:none;
    background:#222; color:#fff; font-size:0.8rem; font-weight:600;
    transition:0.3s;
}
    ...
    <div class="grid" id="albumGrid">
        ...
        <div class="album-card" data-search="<?= strtolower($row['title'].' '.$row['artist']); ?>">
            <div class="media-wrapper">
                <img src="<?= $cover ?>" class="thumb-img" id="thumb-<?= $row['id'] ?>">
                <button class="play-btn"><i class="bi bi-play-fill"></i></button>
                <div class="custom-controls">
                    <input type="range" class="progress" min="0" max="100" value="0" disabled>
                    <button disabled><i class="bi bi-volume-up"></i></button>
                    <button disabled><i class="bi bi-arrows-fullscreen"></i></button>
                </div>
            </div>
