...
<head>
    ...
    <style>
        :root { --bg: #080808; --card: #111; --accent: #ff0055; --accent-grad: linear-gradient(45deg, #ff0055, #ff5e00); }
        ...
        .search-box { background: #151515; border: 1px solid #333; color: white; border-radius: 6px; padding: 6px 15px; width: 250px; }

        /* Grid */
        .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 15px; }
        .video-card { background: var(--card); border-radius: 12px; padding: 12px; border: 1px solid #1a1a1a; transition: 0.3s; position: relative; }
        .video-card:hover { border-color: var(--accent); transform: translateY(-5px); }

        /* Media */
        .media-wrapper { position: relative; width: 100%; aspect-ratio: 1/1; background: #000; border-radius: 8px; overflow: hidden; margin-bottom: 10px; }
        ...
        .title { font-size: 0.85rem; font-weight: 600; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .stars-display { color: #ffca08; font-size: 0.7rem; margin: 5px 0; }
        
        .rev-btn { background: #222; color: #fff; border: none; font-size: 0.7rem; width: 100%; padding: 5px; border-radius: 5px; transition: 0.3s; }
        .rev-btn:hover { background: var(--accent); }

        /* Review Overlay Focus */
        #reviewOverlay {
            display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
            background: rgba(0,0,0,0.9); z-index: 9999; align-items: center; justify-content: center; backdrop-filter: blur(5px);
        }
