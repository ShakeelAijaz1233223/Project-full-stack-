...
// Fetch videos with average rating (Updated to include artist, album, and year)
$query = "SELECT videos.*, 
...
<head>
    ...
    <style>
        ...
        .search-box { background: #1f1f1f; border: 1px solid #333; color: var(--text-main); border-radius: 10px; padding: 8px 16px; width: 280px; transition: all 0.3s; }
        ...
        .video-card { background: var(--card); border-radius: 20px; overflow: hidden; padding: 12px; position: relative; border: 1px solid #2a2a2a; box-shadow: 0 4px 15px var(--shadow); transition: transform 0.3s, box-shadow 0.3s, border-color 0.3s; }
        .video-card:hover { transform: translateY(-6px); border-color: var(--accent); box-shadow: 0 8px 20px var(--shadow); }

        .media-wrapper { position: relative; width: 100%; aspect-ratio: 16/9; background: #000; border-radius: 15px; overflow: hidden; margin-bottom: 12px; }
        .media-wrapper video { width: 100%; height: 100%; object-fit: cover; border-radius: 15px; transition: transform 0.5s ease; }
        .video-card:hover .media-wrapper video { transform: scale(1.07); }

        .play-btn { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 45px; height: 45px; background: var(--accent-grad); border-radius: 50%; border: none; color: #fff; display: flex; align-items: center; justify-content: center; opacity: 0; cursor: pointer; z-index: 5; transition: 0.3s; }
        .video-card:hover .play-btn { opacity: 1; transform: translate(-50%, -50%) scale(1.1); }

        .custom-controls { position: absolute; bottom: 6px; left: 6px; right: 6px; display: flex; align-items: center; justify-content: space-between; padding: 5px 12px; background: rgba(30, 30, 30, 0.75); backdrop-filter: blur(6px); opacity: 0; border-radius: 0 0 12px 12px; transition: opacity 0.3s ease; }
        .media-wrapper:hover .custom-controls { opacity: 1; }
        .custom-controls button { background: none; border: none; color: #fff; cursor: pointer; font-size: 1.2rem; }
        .custom-controls input[type="range"] { flex: 1; margin: 0 6px; accent-color: var(--accent); }

        /* Titles & Categories Styling */
        .title { font-size: 0.95rem; font-weight: 600; margin: 8px 0 2px; color: #fff; }
        .meta-info { font-size: 0.75rem; color: var(--text-muted); margin-bottom: 4px; display: flex; flex-wrap: wrap; gap: 5px; }
        .meta-info span { background: rgba(255,255,255,0.05); padding: 2px 8px; border-radius: 4px; }
        .artist-label { color: var(--accent); font-weight: 500; }

        .stars-display { font-size: 0.78rem; color: #ffd700; margin-bottom: 12px; }

        .rev-btn { background: #2b2b2b; color: #fff; border: none; font-size: 0.75rem; width: 100%; padding: 8px; border-radius: 10px; transition: 0.3s; font-weight: 600; }
...
<body>
    ...
    <div class="studio-wrapper">
        ...
        <div class="grid" id="videoGrid">
            ...
            ?>
                <div class="video-card" data-title="<?= strtolower($row['title'] . ' ' . $row['artist']); ?>">
                    ...
                    <p class="title m-0"><?= htmlspecialchars($row['title']) ?></p>
                    
                    <div class="meta-info">
                        <span class="artist-label"><?= htmlspecialchars($row['artist']) ?></span>
                    ...
                    <button class="rev-btn" onclick="openReview('<?= $row['id'] ?>', '<?= addslashes($row['title']) ?>')">
                        <i class="bi bi-chat-dots-fill me-1"></i> SUBMIT REVIEW
                    </button>
                </div>
            <?php endwhile; ?>
        </div>
    ...
    <script>
        // Search functionality (Updated to filter by Artist too)
        ...
        document.querySelectorAll('.media-wrapper').forEach(wrapper => {
            ...
            video.addEventListener('timeupdate', () => { if (progress) progress.value = (video.currentTime / video.duration) * 100; });
