...
.btn-back {
    ...
    padding: 7px 15px; border-radius: 12px; display: flex;
    align-items: center; gap: 6px; text-decoration: none; transition: 0.3s;
}
...
.album-card {
    background: var(--card); border-radius: 20px; overflow: hidden; padding: 12px;
    position: relative; border: 1px solid #2a2a2a; box-shadow: 0 4px 15px var(--shadow);
...
.disc-wrapper {
    ...
    border-radius: 15px; overflow: hidden; margin-bottom: 12px;
...
.play-btn {
    ...
    box-shadow: 0 0 20px var(--accent); opacity: 0; transition: transform 0.3s, opacity 0.3s;
}
...
.custom-controls {
    ...
    padding: 5px 12px; background: rgba(30,30,30,0.75); backdrop-filter: blur(6px);
    opacity: 0; border-radius: 0 0 12px 12px; transition: opacity 0.3s;
}
